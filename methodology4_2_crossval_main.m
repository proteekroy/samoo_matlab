...
%This file performs k-fold cross valiaditon for Framework 4-2

function  [returnval] = methodology4_2_crossval_main(opt, TrainIndex, TestIndex, k)
    ...
    for i=1:k
        [sep(i), yerror(i)] = methodology4_crossval(opt, opt.archive(TrainIndex{i},:), opt.archive(TestIndex{i},:), ...
            opt.archiveASFAll, TrainIndex{i}, TestIndex{i}, opt.archiveCVModified(TrainIndex{i},:), opt.archiveCVModified(TestIndex{i},:));
    end
...
function [sep, yerror] =  methodology4_crossval(opt, xtrain, xtest, ASF, TrainIndex, TestIndex, cons_train, cons_test)
