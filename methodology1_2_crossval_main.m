...
function  [returnval] = methodology1_2_crossval_main(opt, TrainIndex, TestIndex, k)
    ...
    for i=1:k
        [sep(i), yerror(i)] = methodology1_2_crossval(opt, opt.archive(TrainIndex{i},:), opt.archive(TestIndex{i},:), ...
            opt.archiveObj(TrainIndex{i},:), opt.archiveObj(TestIndex{i},:), opt.archiveCons(TrainIndex{i},:), opt.archiveCons(TestIndex{i},:));
    end
...
function [sep, yerror] =  methodology1_2_crossval(opt, xtrain, xtest, train_obj, test_obj, cons_train, cons_test)
    ...
    index1 = original_cv<=0;%feasible index for test solutions
    ...
    if opt.C>0
        ...
        cons_error = sqrt(sum(((cons_pred-cons_test)./cons_test).^2,2));% this is wrong, divide by sigma, and check whether sigma=1e-14
