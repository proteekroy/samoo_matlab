...
function [selected_pop, selected_pop_obj, selected_pop_cv] = trustedObj_survival_selection(opt, pop, popObj, popCV)
...
if opt.trust_region_option_active==1
    ...
    div = zeros(size(pop,1),1);
    delta =  1;
end 


index = popCV>0;
max_ASF =  max(popObj(~index));
if isempty(max_ASF)
    max_ASF = 0;
end
popObj(index) = max_ASF+ popCV(index);
...
for i = 1:2:N2
    ...
    if div(p1)<=delta && div(p2)<=delta% they are within trust region
        ...
        if div(p1)<=div(p2) %choose the one with nearest 
        	selectedIndividualIndex = p1;
        else
            selectedIndividualIndex = p2;
        end
