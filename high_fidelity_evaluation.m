...
function [f,g] = high_fidelity_evaluation(opt,x)
    ...
    switch(problem)
        ...
        case 'bnh'  
            ...
            %g(2) = -(1/65.3)*((x(1)-8)^2 + (x(2)+3)^2-7.7);%1/7.7*((x(1)-8)^2 + (x(2)+3)^2-7.7);
            g(1) = (1/25)*((x(1)-5)^2 + x(2)^2-25);   
...
%             g(2) = (-1/6)*(6-x(1)-x(2));
%             g(3) = (-1/12)*(2-x(2)+x(1));
%             g(4) = (-1/32)*(2-x(1)+3*x(2));
%             g(5) = (-1/8)*(4-(x(3)-3)^2 -x(4));
%             g(6) = (-1/10)*((x(5)-3)^2+x(6)-4);
        case 'srn'
            f(1) = 2.0+(x(1)-2.0)^2+(x(2)-1.0)^2;
            f(2) = 9.0*x(1)-(x(2)-1)^2;
            g(1) = (1/225)*(x(1)^2+x(2)^2-225.0);
            g(2) = 0.1*(x(1)-3.0*x(2)+10.0);
            %g(1) = (1/575)*(x(1)^2+x(2)^2-225.0);
            %g(2) = (1/30)*(x(1)-3.0*x(2)+10.0);
        case 'tnk'
            f(1) = x(1);
            f(2) = x(2);
            g(1) = (-1)*(x(1)^2+x(2)^2-1.0-0.1*cos(16*atan(x(1)/x(2))));
            g(2) = (1/0.5)*((x(1)-0.5)^2+(x(2)-0.5)^2-0.5);
            %g(1) = (-1/(2*(pi^2)-2))*(x(1)^2+x(2)^2-1.0-0.1*cos(16*atan(x(1)/x(2))));
            %g(2) = (1/(2*(pi^2)-2*pi))*((x(1)-0.5)^2+(x(2)-0.5)^2-0.5);
        case 'water'
		...
		case 'carside'
            ...
            g(1) = (1/0.16)*(1.16 - 0.3717*x(2)*x(4) - 0.0092928*x(3) - 1);
            g(2) = (1/.059)*(0.261 - 0.0159*x(1)*x(2) - 0.06486*x(1) - 0.019*x(2)*x(7) + 0.0144*x(3)*x(5) + 0.0154464*x(6) - 0.32);
            g(3) = (1/0.106)*(0.214 + 0.00817*x(5) - 0.045195*x(1) - 0.0135168*x(1) + 0.03099*x(2)*x(6) - 0.018*x(2)*x(7) + 0.007176*x(3)+ 0.023232*x(3) - 0.00364*x(5)*x(6) - 0.018*x(2)*x(2)- 0.32);
            g(4) = (1/0.42)*(0.74 - 0.61*x(2) - 0.031296*x(3) - 0.031872*x(7) + 0.227*x(2)*x(2)- 0.32);
            g(5) = (1/3.02)*(28.98 + 3.818*x(3) - 4.2*x(1)*x(2) + 1.27296*x(6) - 2.68065*x(7) - 32);
            g(6) = (1/1.86)*(33.86 + 2.95*x(3) - 5.057*x(1)*x(2) - 3.795*x(2) - 3.4431*x(7)+ 1.45728 - 32);
            g(7) = (1/14.36)*(46.36 - 9.9*x(2) - 4.4505*x(1) - 32);
            g(8) = (1/0.72)*(4.72 - 0.5*x(4) - 0.19*x(2)*x(3) - 4);
            g(9) = (10.58 - 0.674*x(1)*x(2) - 0.67275*x(2) - 9.9);
            g(10) = (16.45 - 0.489*x(3)*x(7) - 0.843*x(5)*x(6) - 15.7);
            f(2) = g(8);
            f(3) = 0.5*(g(9)+g(10)+ 9.9+ 15.7);
            g(9) = (1/0.68)*g(9);
            g(10) = (1/0.75)*g(10);
            %}
        ...
        case 'do2dk2'
            javaaddpath(pwd);
            method = KneeProblem;
            method.evaluate_DO2DK2(x)
            f = method.objective;
            f = f';
        case 'deb2dk'
            javaaddpath(pwd);
            method = KneeProblem;
            method.evaluate_DEB2DK(x)
            f = method.objective;
            f = f';
        case 'deb3dk'
            javaaddpath(pwd);
            method = KneeProblem;
            method.evaluate_DEB3DK(x)
            f = method.objective;
            f = f';
