...
function selected_pop = constrained_tournament_selection_nsga3(opt, pop, popObj, popCV)
    ...
    for i = 1:N
        ...
        if (popCV(p1)<=0 && popCV(p2)<=0)%both are feasible 
            ...
            if opt.nsga3.associationsReady && opt.nsga3.pop2Dir(p1) == opt.nsga3.pop2Dir(p2)
                ...
                elseif d == 3 % p2 dominates p1
                    ...
                    if opt.nsga3.pop2DirDistances(p1) < opt.nsga3.pop2DirDistances(p2)
                        pick = p1;
                    else
                        pick = p2;
                    end
